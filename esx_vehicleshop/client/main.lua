	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			elements = {
				...
				{label = _U('test_drive'),  value = 'test_drive'}  -- Hinzugefügt: Option für Testfahrt
			...
			elseif data2.current.value == 'test_drive' then
...
function StartTestDrive(vehicleModel)
	local playerPed = PlayerPedId()
	local pos = vector3(-2097.4995, 997.6998, -26.3639) 
	local heading = 274.6780  
	...
	ESX.Game.Teleport(playerPed, vector3(-2097.4995, 997.6998, -26.3639), function()
		print("Erfolgreich")
	end)

	ESX.Game.SpawnVehicle(vehicleModel, pos, heading, function(vehicle)
		TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
		SetVehicleNumberPlateText(vehicle, "TEST")

		ESX.ShowHelpNotification("Drücke F um die Testfahrt zu beenden.")
		...
		Citizen.CreateThread(function()
			while true do
				...
				if not IsPedInVehicle(playerPed, vehicle, false) then
					ESX.ShowNotification("Testfahrt beendet.")
					break
				end

				if IsControlJustPressed(1, 23) then
					ESX.ShowNotification("Testfahrt beendet.")
					ESX.Game.Teleport(playerPed, vector3(-33.7, -1102.0, 25.4), function()
						print("Erfolgreich")
					end)
